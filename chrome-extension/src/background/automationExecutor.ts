...
import { recordCreditUsage } from '@private-services/subscriptionApi';
...
/**
 * Trigger the credit usage API for custom (non-installed) automations.
 */
const triggerCreditUsageAPI = async (automationName: string) => {
  try {
    const storage = await chrome.storage.local.get(['accessToken', 'persist:uiState', 'persist:root', 'selectedTeamId', 'myCachedAllData']);
    const userId = storage.accessToken;

    // Check if the current active team has local storage mode.
    if (storage.selectedTeamId && Array.isArray(storage.myCachedAllData)) {
      const activeTeam = storage.myCachedAllData.find((t: any) => t.team_id === storage.selectedTeamId);
      if (activeTeam && activeTeam.storageMode === 'local') {
        console.log('[AutomationExecutor] Active team is local, skipping credit usage API');
        return;
      }
    }
    ...
    let personalOrgId = '';
    try {
      // Fetch the full team data to identify the personal space
      if (storage.myCachedAllData && Array.isArray(storage.myCachedAllData)) {
        const personalTeam = storage.myCachedAllData.find((t: any) => t.is_personal_space === true);
        if (personalTeam) {
          personalOrgId = personalTeam.team_id || personalTeam.org_id || '';
        }
      }

      // If personal space wasn't found in cache, fallback to the current selected team
      if (!personalOrgId) {
        if (storage['persist:uiState']) {
          const uiState = JSON.parse(storage['persist:uiState']);
          personalOrgId = uiState.selectedTeam?.team_id || uiState.selectedTeam?.org_id || '';
        }
        if (!personalOrgId && storage['persist:root']) {
          const root = JSON.parse(storage['persist:root']);
          const uiStateRaw = root.uiState;
          const uiState = typeof uiStateRaw === 'string' ? JSON.parse(uiStateRaw) : uiStateRaw;
          personalOrgId = uiState?.selectedTeam?.team_id || uiState?.selectedTeam?.org_id || '';
        }
      }
    } catch (e) {
      console.warn('[AutomationExecutor] Failed to resolve personalOrgId', e);
    }

    if (!userId) {
      console.warn('[AutomationExecutor] No userId found, skipping credit usage API');
      return;
    }
    await recordCreditUsage(userId, personalOrgId);
  } catch (err) {
    console.error('[AutomationExecutor] Credit usage API failed:', err);
  }
};
...
export const executeAction = async (
  ...
  switch (baseAction) {
    ...
    // ── insert_text ───────────────────────────────────────────────────────────
    case 'insert_text': {
      const result = await interactionEngine.type({ tabId, target, action, value: resolvedValue, timeout: action.timeout_ms || 8000 });
      if (result.status === 'failed') {
    ...
    case 'clipboard_paste': {
      const result = await interactionEngine.clipboardRead({ tabId, target, action, timeout: action.timeout_ms || 8000 });
      if (result.status === 'failed') {
  ...
  if (action.expectedOutcome) {
    ...
    while (Date.now() - start < timeout) {
      ...
      const res = await interactionEngine.validateState({
        ...
        expected: {
          ...
          element_present: action.expectedOutcome.type === 'element_appear' ? action.expectedOutcome.selector : undefined,
          element_absent: action.expectedOutcome.type === 'element_disappear' ? action.expectedOutcome.selector : undefined,
    ...
    if (!validated) {
      ...
      throw new Error(`Outcome validation failed: ${action.expectedOutcome.type}. The page did not reach the expected state.`);
    }
  }
...
export const executeAutomation = async (automation: BgSavedAutomation) => {
  ...
  try {
    ...
    if (!automation.id || !automation.id.startsWith('module-')) {
      triggerCreditUsageAPI(automation.name);
    }
      ...
      if (step.moduleId === 'open_tab' || step.moduleId === 'open_url') {
        ...
        if (currentTabId) {
          ...
          if (step.moduleId === 'click') {
            ...
            await executeAction(currentTabId, { 
              action: 'key_press', 
              key, 
