...
const NotesPopup: React.FC<NotesPopupProps> = ({ notes, position, onSelect, onClose, onEdit, externalQuery = '' }) => {
  ...
  // Close the popup automatically when no results remain
  useEffect(() => {
    if (filteredNotes.length === 0) {
      onClose();
    }
  }, [filteredNotes.length, onClose]);
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div className="note-title" style={{ flex: 1 }}>
                      {(() => {
                        const c = note.category?.toLowerCase();
                        const isLink = c === 'link' || c === 'links' || c === 'quicklink';
                    ...
                    <span
                      ...
                      {(() => {
                        ...
                        if (cat === 'links' || cat === 'quicklink' || cat === 'link') {
                          return (
                            <>
                              <svg
                                width="10"
