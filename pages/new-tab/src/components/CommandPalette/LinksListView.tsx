...
import { FaEdit, FaCheck, FaTimes, FaLink, FaNetworkWired } from 'react-icons/fa';
import { FiLoader } from 'react-icons/fi';
import { useSelector, useDispatch } from 'react-redux';
import StackedLinkIcon from '../Shared/Icons/StackedLinkIcon'; // Import StackedLinkIcon
import { selectAllData, selectAllDataLoading } from '../../../../Redux/AllData/allDataSlice'; // Correct path for new-tab
import { setCommandStatus, resetCommandStatus, selectDarkMode } from '../../../../Redux/AllData/uiStateSlice';
...
// Types adapted for new-tab context
interface Team {
  workspaces?: Workspace[];
}
interface Workspace {
  workspace_id: string;
  workspace_snippets?: any[];
  folders?: Folder[];
}
interface Folder {
  folder_id: string;
...
type CommandListRow = {
  ...
  name: string;
  shortcut: string;
  urlLabel: string; // friendly label
  keywords: string[];
...
const parseKeywords = (s: string) =>
  s
    ...
    .map(k => k.trim())
    .filter(Boolean);
// For links view, keep empty keywords visually blank instead of showing a dash
...
const readStorage = async (): Promise<LinkCommandMap> => {
...
const writeStorage = async (map: LinkCommandMap) => {
  const chromeAny = (window as any)?.chrome;
  if (!chromeAny?.storage?.local) return;
  return await new Promise<void>(resolve => {
    chromeAny.storage.local.set({ link_commands: map }, () => resolve());
  });
};
  ...
  for (const team of teams || []) {
    for (const ws of team.workspaces || []) {
      ...
      for (const snippet of ws.workspace_snippets || []) {
        ...
        // Check if quicklink is actually a group
        if (category === 'quicklink' && parsed && Array.isArray(parsed.urls) && parsed.urls.length > 1) {
          isGroup = true;
        }

        if (!isGroup) {
          // It is a single link (or a quicklink with 1 url)
          urlLabel = (snippet.value as string) || '';

          // If it was parsed successfully and has a url/urls field, use that
          if (parsed) {
          ...
          // It matches tabgroup or group-like quicklink
          const urls = (parsed && parsed.urls) || [];
        ...
        rows.push({
          ...
          iconHost,
          isEditable: true,
          type: isGroup ? 'tabgroup' : 'link',
          rawValue: snippet.value,
...
const Row = memo(function Row({ index, style, data }: { index: number; style: React.CSSProperties; data: RowData }) {
  ...
  const handleRowClick = (e: React.MouseEvent) => {
    ...
    // Just update selection, don't open link
    // Selection is handled by parent component
  };
  ...
  return (
    <div
      ...
      <div className="w-1/4 flex items-center gap-2">
        {row.type === 'tabgroup' ? (
          <StackedLinkIcon
            urls={safeParseTabs(row.rawValue)?.urls || []}
            maxIcons={3}
            size={16}
            className="flex-shrink-0"
          />
        ) : row.iconHost ? (
          <img src={getFaviconUrl(row.iconHost)} className="w-4 h-4 rounded-sm flex-shrink-0" alt="" />
        ) : (
          <FaLink className="text-[var(--color-iconDefault)] w-4 h-4 flex-shrink-0" />
        )}
        <div
      ...
      {/* Hot Keys Column */}
      <div className="w-[15%] px-2">{renderHotkeyCell(row.id, row.hotkey)}</div>
    </div>
  );
});

const LinksListView: React.FC<LinksListViewProps> = ({ searchQuery }) => {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const [initial, allHotkeys] = await Promise.all([readStorage(), readAllHotkeys()]);
      if (!mounted) return;
      setLinkCommandsMap(initial);
  ...
  const linkRows = useMemo(() => {
    const rows = buildLinkRows(allData || [], linkCommandsMap || {}, hotkeysMap || {});
  ...
  }, [allData, linkCommandsMap, hotkeysMap, searchQuery]);
  ...
  useEffect(() => {
    setSelectedIndex(prev => {
      const maxIndex = Math.max(0, linkRows.length - 1);
      const clamped = Math.min(prev, maxIndex);
      if (clamped !== prev || linkRows.length === 0) {
        (listRef.current as any)?.scrollToItem(0);
        return 0;
      }
      return clamped;
    });
  }, [linkRows.length, searchQuery]); // Reset on search or data change

  // Removed handleLinkRowClick - no longer opening links on click, just selection
    ...
    (e: React.KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        setSelectedIndex(prev => {
          ...
          const next = prev >= linkRows.length - 1 ? 0 : prev + 1;
        ...
        setSelectedIndex(prev => {
          ...
          const next = prev <= 0 ? linkRows.length - 1 : prev - 1;
          (listRef.current as any)?.scrollToItem(next);
          return next;
        });
      } else if (e.key === 'Enter') {
        e.preventDefault();
        // Just keep selection, don't open link
        // User can use shortcuts to open links
      }
    },
    [linkRows.length],
  );
  ...
  const handleHotkeyUpdate = useCallback(
    async (commandId: string, newValue: string) => {
      ...
      const row = linkRows.find(r => r.id === commandId);
      ...
      try {
        ...
        await updateLocalHotkey(commandId, newValue, 'link');

        // Also save URL data to link_commands so hotkey can open the link
        if (row) {
  ...
  const saveEdit = useCallback(
    async (commandId: string, field: EditField) => {
      ...
      try {
        ...
        // Find the row to get all its data for initialization
        const row = linkRows.find(r => r.id === commandId);
        if (!row) {
          dispatch(setCommandStatus({ status: 'error', message: 'Link not found' }));
          setTimeout(() => dispatch(resetCommandStatus()), 3000);
          return;
        }

        // Preserve all existing fields, or initialize from row data if new entry
        const entry = current[commandId]
          ? { ...current[commandId] }
          : {
              label: row.name,
              url: row.type === 'link' ? (typeof row.rawValue === 'string' ? row.rawValue : '') : '',
              iconHost: row.iconHost || null,
              type: row.type || 'link',
              snippetId: commandId.split('-').slice(-1)[0], // Simplified extraction
            };
        ...
        if (field === 'shortcut') {
          ...
          if (shortcutValue && conflict.isDuplicate) {
            ...
            if (conflict.type === 'command') {
              ...
              conflictMsg = 'Shortcut taken by another note';
        ...
        // For tabgroups, preserve urls if they exist
        if (row.type === 'tabgroup' && row.rawValue) {
          try {
            const val = typeof row.rawValue === 'string' ? JSON.parse(row.rawValue) : row.rawValue;
            if (val?.urls && Array.isArray(val.urls)) {
              entry.urls = val.urls;
            }
          } catch {}
        }
        ...
        if (field === 'shortcut') {
          ...
          try {
            ...
            // Update local storage for fast access
            await updateLocalShortcut(commandId, snippetId, entry.shortcut || '', row.name, 'link', row.type);

            // Update local state
            setLinkCommandsMap(prev => ({
              ...prev,
          ...
          setLinkCommandsMap(current);
          setEditing(null);
          dispatch(setCommandStatus({ status: 'success', message: `${field} updated` }));
          setTimeout(() => dispatch(resetCommandStatus()), 3000);
        }
      } catch (error) {
        console.error(`Failed to save ${field} for link ${commandId}:`, error);
    ...
    [editing, linkRows, hotkeysMap, dispatch],
  );
          ...
          {saving ? (
            <div className="flex items-center gap-1.5 px-2 py-1">
              ...
              <span className="text-[10px] font-medium text-blue-500 whitespace-nowrap">
                {field === 'shortcut'
                  ? linkCommandsMap[commandId]?.shortcut
                    ? 'Updating...'
                    : 'Saving...'
                  : 'Saving...'}
              </span>
  ...
  const rowData = useMemo(
    ...
    [linkRows, renderInput, isEditing, startEdit, selectedIndex, renderHotkeyCell, isDarkMode],
  );
