...
interface FavoriteItemProps {
  ...
  onStartExistingSession?: (suggestion: { snippet: Snippet; workspace: any; folder: any }) => void;
...
const SnippetFavoriteItem: React.FC<FavoriteItemProps & { snippet: Snippet }> = props => {
  ...
  const snippet = useMemo(() => {
    if (!allData || allData.length === 0) return propSnippet;

    const targetId = propSnippet.id || propSnippet.snippet_id;
    const orgId = propSnippet.org_id || '';
    const workspaceId = propSnippet.workspace_id || '';
    const folderId = propSnippet.folder_id || '';

    // Helper: search folders deep
    const searchFoldersDeep = (folders: any[]): any | null => {
      for (const folder of folders) {
        const snippets = folder.snippets || folder.folder_snippets || [];
        const match = snippets.find((s: any) => s.id === targetId || s.snippet_id === targetId);
    ...
    if (orgId && workspaceId) {
      const org = allData.find(t => String((t as any).id || t.team_id) === String(orgId));
      if (org) {
        const ws = org.workspaces?.find((w: any) => String(w.workspace_id || w.id) === String(workspaceId));
        if (ws) {
          if (folderId) {
            const matchInFolders = searchFoldersDeep(ws.folders || []);
            if (matchInFolders) return { ...propSnippet, ...matchInFolders };
          } else {
            // Workspace level snippet
            const wsSnippets = ws.workspace_snippets || [];
            const match = wsSnippets.find((s: any) => s.id === targetId || s.snippet_id === targetId);
            if (match) return { ...propSnippet, ...match };

            // Workspace level automation
            const wsAutos = ws.workspace_automations || [];
            const aMatch = wsAutos.find((a: any) => String(a.id) === String(targetId));
            if (aMatch) return { ...propSnippet, ...aMatch, type: 'automation', category: 'automation' };
          }
        }
      }
    }
    ...
    for (const org of allData) {
      const curOrgId = (org as any).id || org.team_id || '';
      for (const ws of org.workspaces || []) {
        const curWsId = ws.workspace_id || (ws as any).id || '';

        // Workspace-level snippets
        const wsSnippets = ws.workspace_snippets || [];
        const match = wsSnippets.find((s: any) => s.id === targetId || s.snippet_id === targetId);
        if (match) {
          return { ...propSnippet, ...match };
        }
        ...
        const aMatch = wsAutos.find((a: any) => String(a.id) === String(targetId));
        if (aMatch) {
          return { ...propSnippet, ...aMatch, type: 'automation', category: 'automation' };
        }

        // Folders deep
        const folderMatch = searchFoldersDeep(ws.folders || []);
        if (folderMatch) {
          return { ...propSnippet, ...folderMatch };
        }
      }
    }
    ...
    <>
      <motion.div
            ...
            const runClickAction = () => {
              ...
              if (urlsToOpen.length > 0) {
                ...
                if (props.onSnippetSelect) {
                  ...
                  // Build breadcrumb so RichEditor knows where to load/save the note
                  const breadcrumb: NewSnippetBreadCrum = {
                  ...
                  dispatch(setSelectedSnippet(snippet));
                  dispatch(setIsCreatingNewItem(false));
                  dispatch(navigateToView({ kind: 'noteEditor', noteProps: { category: category === 'snippet' ? 'snippet' : 'note' } }));
                }
              } else {
        ...
        <div className="flex items-center gap-1.5 h-[29px] overflow-hidden relative">
                ...
                {AI_GROUP.members.slice(0, 4).map((id, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className="w-4 h-4 rounded-full flex items-center justify-center overflow-hidden border border-white dark:border-neutral-800 bg-[var(--color-iconDefault)] shadow-sm flex-shrink-0 relative" style={{ zIndex: 4 - idx }}>
          ...
          {(isLinkCategory || isTabGroupCategory || snippet.category === 'link' || snippet.category === 'tabgroup' || snippet.category === 'tab group' || snippet.category === 'bulk_link') && (
            <div className="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150 pr-2">
              ...
              <div
                className={`p-1 rounded-full cursor-pointer transition-colors ${
                  isDarkMode ? 'hover:bg-white/10 text-neutral-400 hover:text-white' : 'hover:bg-black/5 text-neutral-500 hover:text-black'
                }`}
                title="Start Session Mode"
                onClick={(e) => {
                  e.stopPropagation();
                  if (onStartExistingSession) {
                    onStartExistingSession({ snippet, workspace, folder });
                  }
                }}
              >
                <FiLayers size={12} />
