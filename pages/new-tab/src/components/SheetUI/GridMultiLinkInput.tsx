...
interface GridMultiLinkInputProps {
  initialUrls: string[];
...
export const GridMultiLinkInput: React.FC<GridMultiLinkInputProps> = ({
  initialUrls,
  onSave,
  onCancel,
  ...
  const onInputKeyDown = (e: React.KeyboardEvent, index: number) => {
    ...
    if (isSearchUIActive) {
      ...
      if (e.key === 'Escape') {
        e.preventDefault();
        e.stopPropagation(); // Stop the global SheetUI listener from closing the editor
    ...
    if (e.key === 'Enter') {
      e.preventDefault();
      e.stopPropagation(); // Stop global SheetUI listener from closing the editor
      if (e.shiftKey) {
        handleAddUrl();
      } else {
        handleFinalSave();
  ...
  return (
    <div
          ...
          return (
            <div
              ...
              className={clsx(
                ...
                isDarkMode ? 'border-b border-white/10 bg-[#000000]' : 'border-b border-slate-100 bg-white',
              ...
              <div className="flex items-center gap-0 group">
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    isDarkMode ? "text-neutral-500 hover:text-red-400" : "text-slate-300 hover:text-red-500"
                  )}
                  title="Remove row">
                  <FaTrash size={9} />
                </button>
              ...
              {showSuggestions && activeQuery.trim().length > 0 && (
                <div
                  className={clsx(
                    ...
                    isDarkMode ? 'bg-[#000000] border-white/20' : 'bg-white border-slate-200',
                    ...
                    {suggestions.map((s, sIdx) => (
                      <div
                        ...
                        className={clsx(
                          ...
                          isDarkMode ? 'border-b border-white/5' : 'border-b border-slate-50',
                          suggestionCursor === sIdx
                            ? (isDarkMode ? 'bg-white/10 text-white' : 'bg-blue-600 text-white')
                            : isDarkMode
                        ...
                        <div className="flex flex-col min-w-0 flex-1">
                          <span
                            className={clsx(
                              ...
                              suggestionCursor === sIdx ? 'text-white' : (isDarkMode ? 'text-neutral-200' : 'text-slate-700'),
                            )}>
                            {s.name}
                          </span>
                          <span
      ...
      <div className={clsx('flex justify-center py-1.5 border-t', isDarkMode ? 'border-white/10' : 'border-slate-50')}>
        <button
          type="button"
          ref={plusButtonRef}
          onClick={e => {
          ...
          className={clsx(
            ...
            isDarkMode
              ...
              : 'bg-emerald-50 hover:bg-emerald-500 text-emerald-600 hover:text-white border border-emerald-200',
          )}
