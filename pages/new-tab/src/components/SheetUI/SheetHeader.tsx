...
import type { RowData, AutomationModuleRow } from './types';
import Resizer from './Resizer';
import { FiArrowUp, FiArrowDown, FiSearch } from 'react-icons/fi';
import { useGridStore } from './gridStore';
import { useDispatch } from 'react-redux';
import { setPendingLockedCommand } from '../../../../Redux/AllData/uiStateSlice';
...
const HeaderContent: React.FC<{
  ...
  isDarkMode: boolean;
  onClose?: () => void;
}> = ({ header, selectedCell, colIndex, columnFilters, setColumnFilter, setSelectedCell, table, isDarkMode, onClose }) => {
    ...
    return (
      <div
        ...
        <input
          ...
          onKeyDown={e => {
            if (e.key === 'Enter') {
              // Let the global handler in SheetTable handle this to ensure correct data row indexing
              return;
...
const SheetHeader: React.FC<SheetHeaderProps> = ({ table, isDarkMode, tutorialStep, setTutorialStep, onClose }) => {
  const { columnFilters, setColumnFilter, selectedCell, setSelectedCell } = useGridStore();
        ...
        return (
          <tr key={headerGroup.id}>
            {headerGroup.headers.map((header, index) => {
              ...
              const isActiveCol = isLeafRow && selectedCell?.colIndex === index && index !== 6;
                  ...
                  className={clsx(
                    ...
                    isLeafRow &&
                      clsx(
                        ...
                        isActiveCol 
                          ? (isDarkMode ? 'text-neutral-300 bg-white/5' : 'text-slate-700 bg-slate-900/5')
                          : 'text-neutral-400 bg-[var(--color-sheetBg)] hover:bg-white/5',
                      ),
                    isLeafRow && index !== 6 && (
                      isDarkMode ? 'border-b border-white/10' : 'border-b border-[#e1e1e1]'
                    ),
                    isLeafRow && isActiveCol && (isDarkMode ? 'bg-white/5' : 'bg-slate-900/5'),
                    index === 6 && 'p-0 text-center',
                  )}>
