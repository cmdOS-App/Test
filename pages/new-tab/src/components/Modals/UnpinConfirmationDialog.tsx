...
const UnpinConfirmationDialog: React.FC<UnpinConfirmationDialogProps> = ({
  ...
  return createPortal(
    <AnimatePresence>
      {(isOpen || isAnimating) && (
        <div className="fixed inset-0 flex items-center justify-center" style={{ zIndex }} onClick={handleOutsideClick}>
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <motion.div
            ...
            className="relative bg-[var(--color-containerBg)] rounded-lg shadow-xl w-full max-w-2xl p-6 border border-[var(--color-borderDefault)]"
            style={{ zIndex: zIndex + 1 }}
            onClick={e => e.stopPropagation()}>
            <div className="flex items-start mb-4">
              <div className="mr-4 bg-gray-100 dark:bg-gray-900/30 p-3 rounded-full shrink-0">
                <BsPinAngleFill className="text-[var(--color-iconDefault)]" size={20} />
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-[var(--color-textPrimary)] mb-2">Unpin cmdOS?</h3>
            ...
            <div className=" border border-gray-200 dark:border-gray-400 rounded-lg p-4 mb-6 flex flex-col gap-6">
              <div className="w-full">
                <div className="flex items-start gap-3 mt-1">
                  <FaInfoCircle className="text-[var(--color-iconDefault)] shrink-0 mt-1" size={18} />
              ...
              {pinImageUrl ? (
                <div className="w-full flex items-center justify-center bg-[var(--color-popupBg)] p-4 rounded-lg border-2 border-[var(--color-borderDefault)] shadow-lg">
                  <img
                    src={pinImageUrl}
                ...
                <div className="w-full max-w-[480px] h-[360px] mx-auto flex items-center justify-center bg-[var(--color-popupBg)] p-4 rounded-lg border-2 border-dashed border-[var(--color-borderDefault)]">
            ...
            <div className="flex justify-end space-x-3">
              <motion.button
                ...
                className="px-4 py-2 rounded-md text-sm font-medium transition-colors bg-[var(--color-containerBg)] hover:bg-neutral-200 dark:hover:bg-neutral-600 text-neutral-700 dark:text-neutral-200 text-white">
                ⚠️ Unpin
              </motion.button>
              <motion.button
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                onClick={handleClose}
                className="px-4 py-2 rounded-md text-sm font-medium transition-colors bg-[var(--color-containerBg)] hover:bg-neutral-200 dark:hover:bg-neutral-600 text-green-700 dark:text-green-400">
                Keep Pinned {`->`}
              </motion.button>
            </div>
          </motion.div>
