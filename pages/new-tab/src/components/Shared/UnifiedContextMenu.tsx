...
import { setHighlightedCommandId, setIsCommandListView, selectIsMac } from '../../../../Redux/AllData/uiStateSlice';
...
import { readAllHotkeys, readAllShortcuts, extractSnippetIdFromCompoundId } from './utils/hotkeyUtils';
...
export const UnifiedContextMenu: React.FC<UnifiedContextMenuProps> = ({
  ...
  const findConflictingItemName = (conflictingId: string): { name: string; type: string } | null => {
    const cmd = COMMANDS.find(c => c.id === conflictingId) || LOCAL_COMMANDS.find(c => c.id === conflictingId);
  ...
  useEffect(() => {
    const runValidation = async () => {
      ...
      if (hotkeyInput?.value) {
        // 1. Check Reserved
        const reserved = checkReservedShortcut(hotkeyInput.value, isMac);
  ...
  }, [hotkeyInput?.value, shortcutInput?.value, itemId, isMac, allData]);
  ...
  return createPortal(
    <div
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-xs flex items-center justify-between gap-2 transition-colors ${isSelected
                        ? 'bg-slate-100 dark:bg-purple-900/20 text-blue-600 dark:text-purple-300 font-medium'
                        : isFocused
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="truncate">
                          {searchQuery ? highlightMatch(action.label || '', searchQuery) : action.label}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        {(action as any).checked && (
                          <FiCheck className="text-purple-600 dark:text-purple-400" size={14} />
                        )}
                        {!action.divider &&
            ...
            {error && conflictId && conflictId !== 'extension-reserved' && showAllHotkeysOption && (
              <div className="px-2 pb-2 pt-1 mt-auto border-t border-slate-100 dark:border-white/5 flex flex-col gap-1">
                <button
                  onClick={e => {
                    e.stopPropagation();
                    if (conflictId) dispatch(setHighlightedCommandId(conflictId));
                    dispatch(setIsCommandListView(true));
                  }}
                  className="w-full flex items-center gap-2 px-2 py-1.5 text-[10px] text-slate-400 dark:text-neutral-400 hover:text-blue-500 dark:hover:text-purple-400 hover:bg-slate-50 dark:hover:bg-neutral-700/50 rounded-md transition-all group">
      ...
      {(hotkeyInput || shortcutInput || rightPanelContent) && (
        <div
          className={`min-w-[260px] w-fit flex flex-col bg-slate-50/50 dark:bg-black/20 animate-in slide-in-from-left-4 duration-300 ${!isInputMode ? 'border-l border-slate-100 dark:border-white/10' : ''}`}>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div
                  className={`flex flex-col rounded-lg overflow-hidden border transition-all duration-200 ${error
                    ? 'border-red-300 dark:border-red-800/50 shadow-[0_0_0_1px_rgba(252,165,165,0.3)] animate-shake'
                    : 'border-purple-400 shadow-sm ring-1 ring-purple-400/20'
                    }`}>
                  ...
                  <div className="px-1 py-1.5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between overflow-hidden">
                    <div className="text-[10px] font-bold tracking-wider text-slate-600 dark:text-neutral-200 truncate pr-2">
                      {hotkeyInput.value
                    ...
                    {hotkeyInput.value && hotkeyInput.onClear && (
                      <button
                        ...
                        className="text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 transition-colors p-1 rounded-md hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-1.5 text-[10px] font-medium"
                  ...
                  {error && (
                    <div className="px-3 py-2 border-t border-red-200/50 dark:border-red-900/30 flex flex-col gap-1">
                        ...
                        <div className="text-[11px] font-medium leading-tight flex flex-wrap gap-x-1">
                          ...
                          {(() => {
                            ...
                            if (parts.length >= 5) {
                              ...
                              else if (typeRaw === 'tabgroup' || typeRaw === 'tab group') typeDisplay = 'Tab Group';
                              else if (typeRaw === 'prompt') typeDisplay = 'Prompt';
                ...
                <div className="flex items-center gap-1.5 px-1 self-end mt-4">
                  {hotkeyInput.showSuccess ? (
                    ...
                    <>
                      ...
                      <span className="text-[10px] font-medium text-purple-500 whitespace-nowrap">
            ...
            <div className="px-3 pb-3 flex flex-col h-full justify-between">
              <div className="flex flex-col gap-3">
                ...
                <div
                  className={`flex flex-col rounded-lg overflow-hidden border transition-all duration-200 ${error
                    ? 'border-red-300 dark:border-red-800/50 shadow-[0_0_0_1px_rgba(252,165,165,0.3)] animate-shake'
                    : 'border-[var(--color-borderDefault)] focus-within:border-blue-500 dark:focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500/30 dark:focus-within:ring-blue-500/30'
                    }`}>
                  {/* Header */}
                  <div className="px-2 py-1 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
                    ...
                    {shortcutInput.value && shortcutInput.onClear && (
                      <button
                        ...
                        className="text-[var(--color-iconDefault)] hover:text-red-500 transition-colors p-1 flex items-center justify-center rounded-md hover:bg-slate-100 dark:hover:bg-white/10"
                  ...
                  {/* Footer (Error & Link - Minimalist) */}
                  {error && (
                    <div className="px-2 py-1.5 border-t border-red-200/50 dark:border-red-900/30 flex flex-col gap-1">
