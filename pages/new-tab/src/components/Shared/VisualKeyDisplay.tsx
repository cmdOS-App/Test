...
export const VisualKeyDisplay = ({
  ...
  if (variant === 'text') {
    return (
      <div className={clsx(
        "flex items-center gap-1 font-medium text-[11px] tracking-tight",
  ...
  return (
    <div className="flex items-center gap-1 pointer-events-none select-none">
      {keys.map((k, i) => {
        if (k === '+')
          return (
            <span key={i} className={`text-neutral-400 ${currentSize.plus}`}>
              +
            </span>
          );
        ...
        const colorClasses =
          variant === 'black'
            ? 'bg-[#222] border-none text-white shadow-md'
            : !isDarkMode
              ? 'bg-[#eee8d5] border-[#93a1a1] text-[#073642]'
              : 'bg-[var(--color-containerBg)] border-[var(--color-borderDefault)] text-neutral-700 dark:text-neutral-300';
