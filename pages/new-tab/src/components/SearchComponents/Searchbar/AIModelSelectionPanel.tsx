...
import { getFaviconUrl, stripCmdStatus } from './utils';
...
interface AIModelSelectionPanelProps {
...
const AIModelSelectionPanel: React.FC<AIModelSelectionPanelProps> = ({
  state,
  isMac = false,
  savedAgents = [],
  onSaveAgent,
      ...
      <div className="flex-1 flex flex-col justify-center items-center min-h-0 w-full relative z-10 pb-[50px] px-0.5 transition-all duration-300">
        <div
          className={`w-full border rounded-[5px] overflow-hidden shadow-2xl group/models ${
            isDarkMode ? 'border-white/20 bg-black/40 backdrop-blur-md' : 'border-[#d8d2bf] bg-[#fdf6e3]'
          }`}>
          <div
            className={`py-1 border-b px-1 ${isDarkMode ? 'border-white/20 bg-white/5' : 'border-[#d8d2bf] bg-[#eee8d5]/60'}`}>
            <h3
              className={`text-[11px] font-black tracking-[0.2em] text-left ${isDarkMode ? 'text-white/50' : 'text-[#586e75]'}`}>
              Select models
            </h3>
          </div>
          {modelWarning && (
          ...
          <div className="overflow-y-auto no-scrollbar max-h-[380px] w-full flex flex-col items-stretch">
                ...
                <div key={model.id} className={`flex flex-col last:border-b-0 ${''}`}>
                  <motion.button
                    ...
                    className={`group/modelitem grid grid-cols-[20px_1fr_20px] items-center py-1 px-0.5 transition-all duration-300 w-full ${
                      isActive ? (isDarkMode ? 'bg-white/[0.03]' : 'bg-[#eee8d5]/80') : ''
                    } cursor-pointer`}>
                      ...
                      <div className="flex items-center gap-1.5 w-full">
                        ...
                        {!editingModelId && (
                          <button
                            ...
                            className={`p-1 rounded opacity-0 group-hover/modelitem:opacity-100 transition-all hover:bg-white/10 ${isDarkMode ? 'text-white/40' : 'text-[#93a1a1]'}`}>
                            <FaPencilAlt size={8} />
                          </button>
                        )}
                      </div>
                      {editingModelId === model.id ? (
                        <textarea
                          ...
                          className={`text-[8px] leading-[10px] w-full mt-0.5 bg-transparent border border-solid rounded p-1 outline-none resize-none overflow-y-auto no-scrollbar shadow-inner transition-all ${
                            isDarkMode ? 'text-white/80 border-white/20' : 'text-[#073642] border-[#d8d2bf]'
                          }`}
                        />
                      ) : (
                        <div className="flex items-center gap-1 w-full group/urlwrap">
                          <span
                            className={`text-[8px] font-medium block break-all flex-1 mt-0.5 text-left ${
                              isDarkMode ? 'text-white/40' : 'text-[#93a1a1]'
                            }`}>
                    ...
                    <div className="flex items-center justify-center relative transition-all">
                      <div
                        ...
                        {isActive && <FaCheck size={6} className={isDarkMode ? 'text-white' : 'text-[#073642]'} />}
                      </div>
                    </div>
                  </motion.button>
                </div>
            ...
            {customModels.length > 0 && (
              <div className={`border-t my-1 ${isDarkMode ? 'border-white/10' : 'border-[#d8d2bf]'}`} />
            )}
            {customModels.map((custom: any) => {
              ...
              return (
                <div key={custom.id} className="flex flex-col last:border-b-0">
                  {isEditing ? (
                    <div
                      className={`flex flex-col gap-1 p-1.5 transition-all ${isDarkMode ? 'border-white/10' : 'border-[#d8d2bf]'}`}>
                      <input
                        type="text"
                        ...
                        className={`text-[8px] leading-[10px] p-1 bg-transparent border-b outline-none ${isDarkMode ? 'text-white/80 border-white/20' : 'text-[#073642] border-[#d8d2bf]'}`}
                      />
                      <textarea
                        autoFocus
                    ...
                    <motion.button
                      ...
                      <div className="flex flex-col items-start pl-2 pr-1 min-w-0 py-0.5">
                        <div className="flex items-center gap-1.5 w-full">
                          <span
                            className={`text-[10px] font-bold truncate ${isActive ? (isDarkMode ? 'text-white' : 'text-[#073642]') : isDarkMode ? 'text-white/50 group-hover:text-white/80' : 'text-[#586e75] group-hover:text-[#073642]'}`}>
                            {custom.name}
                          </span>
                          <button
                            onClick={e => {
                      ...
                      <div className="flex items-center justify-center relative transition-all">
                        <div
                          className={`w-3.5 h-3.5 rounded-[4px] transition-all duration-300 flex items-center justify-center flex-shrink-0 ${isActive ? (isDarkMode ? 'bg-white/20' : 'bg-[#eee8d5]') : isDarkMode ? 'border border-white/10 group-hover:border-white/20' : 'border border-[#d8d2bf] group-hover:border-[#93a1a1]'}`}>
                          {isActive && <FaCheck size={6} className={isDarkMode ? 'text-white' : 'text-[#073642]'} />}
                        </div>
                      </div>
                    </motion.button>
            ...
            {isAddingModel ? (
              <div
                className={`flex flex-col gap-1 p-1.5 border-t transition-all ${isDarkMode ? 'border-white/10' : 'border-[#d8d2bf]'}`}>
                <input
                  type="text"
                  placeholder="Name (Required)"
                  value={newModelName}
              ...
              <motion.button
                ...
                className={`flex items-center justify-center py-2 px-1 transition-all duration-300 w-full border-t cursor-pointer opacity-0 group-hover/models:opacity-100 ${
                  isDarkMode ? 'border-white/10 hover:bg-white/[0.03]' : 'border-[#d8d2bf] hover:bg-[#eee8d5]/80'
                }`}>
                <FaPlus size={10} className={isDarkMode ? 'text-white/50' : 'text-[#586e75]'} />
              </motion.button>
            )}
          </div>
        </div>
...
export default AIModelSelectionPanel;
