...
import { getRecentCommands, getFaviconUrl } from './utils';
...
interface AtCommandPopupProps {
  ...
  onGoToTemplates?: () => void; // Navigate to TemplatesView when command not installed
  prompts?: Snippet[];
  isLockedAI?: boolean;
  hideTabs?: boolean;
}
...
const getIconForCommand = (id: string, keywords: string[] = []) => {
  ...
  if (lowerId.includes('prompt')) return LuSparkles;
...
const getColorForCommand = (id: string) => {
  ...
  if (lowerId.includes('prompt')) return 'text-purple-500';
  if (lowerId === 'upload_drive' || lowerId === 'drive') return 'text-blue-500';

  return 'text-neutral-500';
};

// Normalize prompts to command structure
const getPromptCommands = (prompts: Snippet[] = []) => {
  return prompts.map(p => ({
    id: `prompt:${p.id || p.snippet_id}`,
    label: p.key, // snippet key is the title
    icon: LuSparkles,
    color: 'text-purple-500',
    keywords: ['prompt'],
    hidden: false,
    isPrompt: true, // Marker
    value: p.value, // Store content for easy access if needed
  }));
};
...
const AtCommandPopup: React.FC<AtCommandPopupProps> = ({
  ...
  const filteredCommands = useMemo(() => {
    ...
    const defaultFiltered = isLockedAI ? [] : getFilteredAtCommands(searchQuery, prompts, recentIds);
    if (!searchQuery) {
  ...
  }, [searchQuery, prompts, openTabs, isLockedAI, hideTabs]);
...
export const getFilteredAtCommands = (query: string, prompts: Snippet[] = [], recentIds: string[] = []): AtCommandItem[] => {
  const allCommands = getAllAtCommands();
  const promptItems = getPromptCommands(prompts).map(p => ({ ...p, category: 'Prompts' }));
  const searchItems = [...allCommands, ...promptItems];
  ...
  if (!query) {
    ...
    // Limit prompts specifically
    if (categorized['Prompts'].length > MAX_PROMPT_RESULTS) {
      categorized['Prompts'] = categorized['Prompts'].slice(0, MAX_PROMPT_RESULTS);
    }
  ...
  const categoryOrder = ['Recent', 'AI Assistants', 'Workspace', 'Tools', 'Prompts'];
  ...
  // Even when filtered, we still want to cap Prompts to avoid giant menus
  const promptsOnly = sorted.filter((c: AtCommandItem) => c.category === 'Prompts');
  const promptCount = promptsOnly.length;
  if (promptCount > MAX_PROMPT_RESULTS) {
    const nonPrompts = sorted.filter((c: AtCommandItem) => c.category !== 'Prompts');
    const promptsOnlyCap = promptsOnly.slice(0, MAX_PROMPT_RESULTS);
    return [...nonPrompts, ...promptsOnlyCap];
  }

  return sorted;
};

export const getFilteredAtCommandCount = (query: string, prompts: Snippet[] = [], recentIds: string[] = []): number => {
  return getFilteredAtCommands(query, prompts, recentIds).length;
};
