...
import {
  ...
  parseUrlParts,
  assembleUrl,
} from '../../Editor/AgentOptionPopup';
import type { ParamInputType, ParamConfig } from '../../Editor/AgentOptionPopup';
...
import { getInstalledModules, updateAutomation, getUserId, fetchModuleMetadata } from '../../../../../Apis/core/api';
...
import HotkeyAssignButton from '../../Editor/HotkeyAssignButton';
import {
  updateLocalHotkey,
  updateLocalShortcut,
  getUserHotkey,
...
interface SavedAutomationsPanelProps {
  automations: any[];
...
const SavedAutomationsPanel: React.FC<SavedAutomationsPanelProps> = ({
  automations,
  onRunAutomation,
  onEditAutomation,
  onExecuteModule,
  ...
  useEffect(() => {
    const syncFav = async () => {
      ...
      try {
        ...
        console.error('[SavedAutomationsPanel] Fav Sync error:', err);
      }
    };
    syncFav();
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      ...
      console.error('[SavedAutomationsPanel] Toggle Favorite Error:', err);
    }
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        const item = displayItems[selectedIndex];
        if (!item) return;
        if (item.type === 'saved') onEditAutomation?.(item.data);
  ...
  return (
    <div
      className={`relative flex h-[90%] max-h-[90vh] w-full flex-col bg-transparent overflow-hidden border rounded-xl dark:rounded-none ${isDarkMode ? 'border-white/10 dark:border-white/10' : 'border-black/5'}`}>
      <div className="flex flex-1 w-full min-h-0">
        {/* Left Pane: List of Automations (50%) */}
        <div
          className={`w-1/2 flex flex-col border-r ${isDarkMode ? 'border-white/10' : 'bg-white border-black/5'} overflow-hidden`}>
                ...
                {displayItems.map((item, index) => {
                  ...
                  if (item.type === 'installed_category') {
                    ...
                    return (
                      <div
                        ...
                        className={`relative group flex items-center gap-3 border-b px-4 py-2 text-left transition ${isDarkMode ? 'border-white/5' : 'border-black/5'
                          } ${isSelected
                            ? isDarkMode
                              ? focusSection === 'list'
                        ...
                        ${isDarkMode ? 'bg-transparent border border-white/5' : 'bg-white shadow-sm border border-black/5'}
                      `}>
                          {group.iconUrl ? (
                        ...
                        <div className="flex flex-col flex-1 min-w-0">
                          <span
                            className={`truncate text-[12px] font-medium leading-tight ${isDarkMode ? 'text-white' : 'text-[#073642]'}`}
                        ...
                        <div
                          ...
                          <FaChevronRight size={10} className={`${isDarkMode ? 'text-white/20' : 'text-black/20'}`} />
                        </div>
                      </div>
                    );
                  }
                  ...
                  return (
                    <div
                      ...
                      role="button"
                      tabIndex={0}
                      className={`group flex w-full items-center gap-3 border-b px-4 py-2.5 text-left transition-all ${isDarkMode ? 'border-white/5' : 'border-black/5'} ${isSelected ? (isDarkMode ? (focusSection === 'list' ? 'bg-white/10' : 'bg-white/5') : 'bg-white') : isDarkMode ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}>
                      <div
                        className={`flex h-6 w-6 items-center justify-center rounded transition-all ${isDarkMode
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <p
                            className={`truncate text-[12px] font-semibold ${isSelected ? (isDarkMode ? 'text-white' : 'text-[#073642]') : isDarkMode ? 'text-white/90' : 'text-[#073642]'}`}>
                            {toProperCase(data?.name || data?.module_key || 'Untitled Automation')}
                          </p>
                          <span
                            className={`flex-shrink-0 text-[10px] font-medium ${isDarkMode ? 'text-white/30' : 'text-black/40'}`}>
                            {stepLabel}
                          </span>
                        </div>
                      </div>
        ...
        <div className={`w-1/2 flex flex-col overflow-hidden ${isDarkMode ? 'bg-black/20' : 'bg-white'}`}>
          {selectedItem ? (
      ...
      {/* Footer */}
      <div
        className={`flex-none h-8 border-t flex items-center justify-between px-4 z-30 ${isDarkMode ? 'border-white/10 bg-black/90 text-white/40' : 'border-black/5 bg-white text-neutral-500'}`}>
...
const AutomationDashboard: React.FC<AutomationDashboardProps> = ({
      ...
      if (item && currentId) {
        try {
          ...
          const compoundId = getItemCompoundId({
            ...data,
            workspace_id: data.workspace_id || selectedWorkspace?.workspace_id,
            folder_id: data.folder_id || selectedFolder?.folder_id,
            category: 'automation',
          });
  ...
  const handleNameSave = async () => {
    ...
    try {
      if (item.type === 'saved') {
        ...
        await updateAutomation({ id: Number(data.id), name: editedName, steps: apiSteps });
      }
  ...
  const handleHotkeyChange = async (newHotkey: string) => {
    setHotkey(newHotkey);
    try {
      if (item.type === 'saved' && userId) {
        const cloudId = String(data.id || itemId);
        await updateAutomationRealtime(userId, {
          automation_id: cloudId,
      ...
      await updateLocalHotkey(compoundId, newHotkey, item.type === 'saved' ? 'automation' : 'module');
    } catch (err) {
  ...
  const handleShortcutChange = async (newShortcut: string) => {
    ...
    try {
      ...
      if (item.type === 'saved' && userId) {
        ...
        await updateAutomationRealtime(userId, {
          automation_id: cloudId,
          shortcuts: normalizedShortcut || null,
        });
      }
      const compoundId = getItemCompoundId({
        ...data,
  ...
  return (
    <div className="flex flex-col h-full w-full p-5 animate-in fade-in duration-300 overflow-y-auto custom-scrollbar">
      <div className="flex items-center gap-4 mb-4">
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-lg transition-all ${isDarkMode ? 'bg-transparent border border-white/10' : 'bg-white shadow-sm border border-black/5'
            }`}>
        ...
        <div className="flex-1 min-w-0 flex flex-col pt-0.5">
              ...
              {!isEditingName ? (
                <>
                  <h2
                    ...
                    className={`text-[17px] font-bold tracking-tight outline-none truncate transition-all px-2 py-0.5 rounded-lg border-2 ${isFocused && dashboardFocus === 'name'
                        ...
                        : `border-transparent opacity-80 ${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-black/5'}`
                ...
                <div
                  ...
                  <button
                    ...
                    className={`p-1 rounded transition-all active:scale-95 border shadow-sm ${isDarkMode
          ...
          {isInstalled && (
            <p
              className={`text-[11px] font-medium leading-relaxed max-w-sm mt-1 ${isDarkMode ? 'text-white/30' : 'text-black/40'}`}>
      ...
      {!isInstalled && editableStepsExist && (
        <div
          ...
          <div className="flex items-center justify-between px-1">
            <h3
              className={`text-[10px] font-bold tracking-widest opacity-30 ${isDarkMode ? 'text-white' : 'text-black'}`}>
              Configuration
            </h3>
          </div>
              ...
              .map((step, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`relative rounded-xl border transition-all outline-none ${isDarkMode ? 'bg-black/30 border-white/5' : 'bg-white border-black/5'
                    ...
                    <HighlightedInput
                      ...
                      className={`w-full min-h-[32px] text-[11px] font-medium p-2 pb-5 ${isDarkMode ? 'text-white/90' : 'text-neutral-800'}`}
            ...
            <div className="flex min-w-0 items-center gap-2">
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-lg 
                       ${isDarkMode ? 'bg-transparent border border-white/10' : 'bg-white shadow-sm border border-black/5'}
                     `}>
                {category.iconUrl ? (
              ...
              <h4
                className={`truncate text-[15px] font-semibold ${isDarkMode ? 'text-white/90' : 'text-[#073642]'}`}
                style={headingFontStyle}>
                {category.title}
              </h4>
              <p className={`truncate text-[12px] mt-0.5 ${isDarkMode ? 'text-white/40' : 'text-[#586e75]'}`}>
            ...
            {commands.map((command, index) => {
              ...
              return (
                <div
                  ...
                  className={`relative grid grid-cols-[1fr_auto] items-center gap-3 border-b px-2 py-2 transition-colors ${isDarkMode ? 'border-white/10' : 'border-black/10'
                    } ${itemIsSelected
                      ? isDarkMode
                        ? 'bg-white/10'
                        : 'bg-white'
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div
                      className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-md border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-black/5 shadow-sm'
                        }`}>
                      {(() => {
                        ...
                        return <FaTerminal size={18} className={isDarkMode ? 'text-white/40' : 'text-[#586e75]'} />;
                      })()}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p
                        className={`truncate text-[12px] font-medium ${isDarkMode ? 'text-white' : 'text-[#073642]'}`}
                        style={headingFontStyle}>
                        {getCommandDisplayName(command)}
                      </p>
                      <p className={`truncate text-[11px] ${isDarkMode ? 'text-white' : 'text-[#586e75]'} opacity-100`}>
                    ...
                    <div className="flex-shrink-0">
                      <button
                        ...
                        className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-[11px] font-medium transition active:scale-95 ${isDarkMode
