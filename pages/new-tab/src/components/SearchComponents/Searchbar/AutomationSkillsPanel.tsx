...
import { removeRecentCommand } from './utils';
...
interface AutomationSkillsPanelProps {
...
const openModuleInNewTab = (module: ModuleCatalogItem) => {
  ...
  const baseUrl =
    typeof chrome !== 'undefined' && chrome.runtime?.getURL
      ? chrome.runtime.getURL('new-tab/index.html')
      : 'new-tab/index.html';
...
const AutomationSkillsPanel: React.FC<AutomationSkillsPanelProps> = ({
  onExecuteModule,
  query = '',
  activeTab = 'catalog',
  onTabChange,
  ...
  const fetchModules = useCallback(async (forceCloud = false) => {
    ...
    try {
      ...
      const [catalogResult, enrichedInstalled] = await Promise.all([
        getModuleCatalog(),
        syncInstalledModulesToStorage(),
      ]);
  ...
  const handleInstallModule = useCallback(
    async (module: ModuleCatalogItem) => {
      ...
      try {
        ...
        await installModule(moduleId, settings ? { settings } : undefined);
  ...
  const primaryTextColor = isDarkMode ? 'text-white' : 'text-[#073642]';
  const secondaryTextColor = isDarkMode ? 'text-white/45' : 'text-[#586e75]';

  return (
    <div
      className={`relative flex flex-col h-[100%] max-h-[90vh] w-full min-w-0 bg-transparent overflow-hidden border rounded-xl dark:rounded-none ${isDarkMode ? 'border-white/10 dark:border-white/10' : 'border-black/5'
        }`}>
      <div className="flex flex-1 w-full min-h-0">
        {/* LEFT COLUMN: MASTER LIST (40%) */}
        <div
          ref={leftListRef}
          className={`w-[40%] min-w-0 flex flex-col border-r overflow-y-auto custom-scrollbar ${isDarkMode ? 'border-white/10' : 'bg-white border-black/5'
            }`}>
          {/* <div className={`flex items-center justify-between border-b px-4 py-2 ${isDarkMode ? 'border-white/10' : 'border-black/5'}`}>
              ...
              groupedCategories.map((group, index) => {
                ...
                return (
                  <div
                    ...
                    className={`relative group flex items-center gap-3 border-b text-left transition ${isDarkMode ? 'border-white/5' : 'border-black/5'
                      } ${isTarget
                        ? isDarkMode
                          ? 'bg-white/10'
                          : 'bg-white'
                    ...
                    ${isDarkMode ? 'bg-transparent' : 'bg-white shadow-sm border border-black/5'}
                  `}>
                      {group.iconUrl ? (
                    ...
                    <div className="flex flex-col flex-1 min-w-0">
                      <span
                        className={`truncate font-medium leading-tight ${isDarkMode ? 'text-white' : 'text-[#073642]'}`}
                    ...
                    <div
                      ...
                      {!isGroupInstalled ? (
                        ...
                        <div
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border transition-all ${isDarkMode
        ...
        <div
          ref={rightListRef}
          className={`w-[60%] min-w-0 flex flex-col overflow-y-auto custom-scrollbar ${isDarkMode ? 'bg-black/20' : 'bg-white'
            }`}>
                    ...
                    <div className="flex min-w-0 items-center gap-3">
                     ...
                     `}>
                      ...
                      <div className="flex flex-col min-w-0">
                        <h4
                          className={`truncate font-semibold ${isDarkMode ? 'text-white/90' : 'text-[#073642]'}`}
                          style={{ ...headingFontStyle, fontSize: `${dynamicTitleFontSize}px` }}>
                          {selectedCategory.title}
                        </h4>
                        <p
                          className={`truncate mt-0.5 ${isDarkMode ? 'text-white/40' : 'text-[#586e75]'}`}
                    ...
                    {selectedCategory.installedCount === selectedCategory.totalCount && (
                      <div className="flex-shrink-0 animate-in fade-in slide-in-from-left-2 duration-300">
                        <button
                          ...
                          className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-[10px] font-medium transition active:scale-95 ${isDarkMode
                    ...
                    {selectedCategoryCommands.map((command, index) => {
                      ...
                      return (
                        <div
                          ...
                          className={`relative grid grid-cols-[1fr_auto] items-center gap-3 border-b transition-colors ${isDarkMode ? 'border-white/10' : 'border-black/10'
                            } ${isTarget
                              ? isDarkMode
                                ? 'bg-white/10'
                                : 'bg-white'
                          ...
                          <div className="flex items-center gap-2.5 min-w-0">
                            <div
                              ...
                              {(() => {
                                ...
                                return (
                                  <FaTerminal size={11} className={isDarkMode ? 'text-white/40' : 'text-[#586e75]'} />
                                );
                              })()}
                            </div>
                            <div className="min-w-0 flex-1">
                              <p
                                className={`truncate font-medium ${isDarkMode ? 'text-white' : 'text-[#073642]'}`}
                                style={{ ...headingFontStyle, fontSize: `${dynamicFontSize}px` }}>
                                {getCommandDisplayName(command)}
                              </p>
                              <p
                                className={`truncate ${isDarkMode ? 'text-white' : 'text-[#586e75]'} opacity-100`}
...
export default AutomationSkillsPanel;
